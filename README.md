...
- Telegram subscriber self-service:
  - `/status`
  - `/subscriptions`
  - `/payments`
  - `/web`
...
- видит каталог
- проверяет свой статус и доступ через `/status`
- оформляет `stub/manual` checkout
- после активации получает feed по ACL
- при необходимости использует web fallback через Telegram-auth path и verification page
...
- subscriber self-service baseline now includes:
  - `/status`
  - `/subscriptions`
  - `/payments`
  - `/web`
  ...
  - deep link `https://t.me/<bot>?start=web` for faster web verification handoff
  - safe local `next` redirect support in `/tg-auth`
  - inline product selection and callback-based checkout preview/confirm in Telegram
  - self-service overview of active subscriptions and pending payments in bot
...
1. compose cleanup and remaining file-mode parity
