...
from pitchcopytrade.services.moderation import (
    ...
    list_message_audit_events,
    list_moderation_recommendations,
    reject_message,
    send_message_to_rework,
)
...
async def moderation_detail_page(
    ...
    recommendation = await get_moderation_message(session, recommendation_id)
    if recommendation is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    history = await list_message_audit_events(session, recommendation_id)
    metrics = build_moderation_detail_metrics(recommendation, history)
    return templates.TemplateResponse(
        request,
        "moderation/detail.html",
        {
            "title": recommendation.title or "Message moderation",
            "user": user,
...
@router.post("/messages/{recommendation_id}/approve")
async def moderation_approve_submit(
    ...
    if recommendation is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    updated = await approve_message(session, recommendation, user, comment)
    if getattr(updated.status, "value", updated.status) == MessageStatus.PUBLISHED.value:
        ...
        try:
            if session is None:
                ...
                if latest is not None:
                    await deliver_message_notifications_file(graph, store, latest, bot)
            else:
                await deliver_message_notifications(session, updated, bot)
        finally:
            await bot.session.close()
    return RedirectResponse(url=f"/moderation/messages/{recommendation_id}", status_code=status.HTTP_303_SEE_OTHER)


@router.post("/messages/{recommendation_id}/rework")
async def moderation_rework_submit(
    ...
    if recommendation is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    await send_message_to_rework(session, recommendation, user, comment)
    return RedirectResponse(url=f"/moderation/messages/{recommendation_id}", status_code=status.HTTP_303_SEE_OTHER)


@router.post("/messages/{recommendation_id}/reject")
async def moderation_reject_submit(
    ...
    if recommendation is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Message not found")
    await reject_message(session, recommendation, user, comment)
    return RedirectResponse(url=f"/moderation/messages/{recommendation_id}", status_code=status.HTTP_303_SEE_OTHER)
...
def _parse_queue_status(status_value: str):
    ...
    try:
        return MessageStatus(normalized)
    except ValueError:
        return None
